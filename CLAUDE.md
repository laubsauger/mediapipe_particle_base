...
| `logo_grad.frag` | GLSL **TOP** — turns the logo image (`/project1/null_logo`) into a force+mask field for the soup: RGB = ∇(luma) (gradient → attractor direction toward the bright shape), A = luma (the mask). Native Lookup Texture POPs (`logo_force_pop` in the force chain, `c_logo_lookup` in the render chain) sample THIS at each particle's `Puv` → write a 4-comp `logodata` attribute (`.xy` = attract dir, `.w` = mask). No GLSL-POP sampler (would crash). Synced to `logo_grad_pixel`. |
